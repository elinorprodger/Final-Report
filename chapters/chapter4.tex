...
The industrial scenario carries the highest imposed load ($q_k = 7.5$\,kN/m$^2$). The optimal cross-sections for the feasible configurations are listed in Table~\ref{tab:industrial_optima}. Two outcomes distinguish this scenario from the others. First, composite GWP is the highest of any scenario, from 92.0 at 3\,m to 173.2\,kg\,CO$_2$eq/m$^2$ at 9\,m (single span). Second, and more significantly, \textbf{no feasible composite solution exists for the two--span continuous configuration at any span}: under the industrial construction-stage load, the factored wet-concrete weight ($\gamma_Q Q_{k,1c}$) is so large that no deck in the database can carry it over the full span to the permanent supports, even with propped construction reducing the effective sub-span to $L/2$. The governing check in each case is the construction-stage ULS bending resistance of the bare deck. The three-span configuration remains feasible up to 7\,m, where the fire D3 hogging check at the internal support becomes critical; beyond 7\,m no feasible section exists. This infeasibility under industrial loading marks a practical limit of profiled-deck composite construction for that use category.

\begin{table}[h!]
\small\centering
\caption{Optimal composite cross--sections, industrial scenario (two--span infeasible for all spans).}
...
\midrule
\multicolumn{7}{l}{\textit{Two--span: no feasible composite solution at any span (construction-stage}} \\
\multicolumn{7}{l}{\textit{bending governs; deck capacity exceeded under industrial wet-concrete load).}} \\
\midrule
