...
\todo[inline]{Add specific EPD references for each manufacturer once collected. do i need to include all of them?}
...
Vibration serviceability is not incorporated as a structural constraint within the main optimisation. The full SCI~P354 method \citep{RefWorks:hicks2009p354}, which is the principal UK guidance for composite floor vibration, requires bay-level geometry inputs --- secondary beam span, primary beam span, bay width, and damping ratio --- none of which are defined in a slab-only optimisation study. Including a beam model would introduce multiple additional discrete variables (beam section, bay geometry) and fundamentally change the scope of the comparison, which is limited to the slab cross-section.
...
\todo[inline]{ Include a short paragraph on how commonly the full P354 method is used in practice and whether there is further guidance available (SCI P354 is the standard reference; include context from the vibration check results). Add full bay configuration example to appendix.}
...
\todo[inline]{Confirm the precise SIA~181 and UK numeric requirements ($D_i$ and $L'_i$) for each use case; the values are classification-dependent; add table too of the requirements}

\todo[inline]{Add reference to SCI P321 and 322 for acoustic performance of each use case build-up; add indicative airborne ($D_{nT,w}+C_\mathrm{tr}$ / $D_i$) and impact ($L'_{nT,w}$ / $L'_i$) values.}
...
For each validation case, the model-computed maximum permissible span at a given slab depth was compared against the corresponding manufacturer-tabulated value for the same loading and fire rating. Validation was performed across the following deck families: ComFlor~225 and ComFlor~51/60/80 (Tata Steel, EC~4 design tables), ComFlor~210 (Tata Steel, BS~5950 design tables), and Multideck~60/80/146 (Kingspan, BS~5950 design tables). Table~\ref{tab:validation_summary} summarises the results by deck family and span.

\begin{table}[h!]
\small
\centering
\caption{Validation summary: number of passing and failing configurations by deck family. PASS: computed maximum span within manufacturer-stated range; FAIL: computed span exceeds manufacturer limit (non-conservative); N/A: manufacturer table does not cover the propped/gauge combination.}
\label{tab:validation_summary}
\begin{tabular}{lcccc}
\toprule
Deck family & Code & n PASS & n FAIL & n N/A \\
\midrule
ComFlor 225 & EC  & 23 & 2  & 0 \\
ComFlor 51  & EC  & 41 & 3  & 0 \\
ComFlor 60  & EC  & 30 & 25 & 0 \\
ComFlor 80  & EC  & 23 & 20 & 0 \\
ComFlor 210 & BS  & 17 & 6  & 0 \\
Multideck 60  & BS & 31 & 0  & 0 \\
Multideck 80  & BS & 27 & 0  & 0 \\
Multideck 146 & BS & 15 & 0  & 37 \\
\bottomrule
\end{tabular}
\end{table}

Deep deck profiles (ComFlor~225, Multideck~60/80/146, ComFlor~210) show consistently good agreement with manufacturer tables: the computed permissible spans are within the tabulated range for the large majority of configurations, with the few failures concentrated at longer spans where the manufacturer table ends and the model applies the standard Eurocode equations beyond the tabulated range.

The shallow deck families ComFlor~60 and ComFlor~80 exhibit a higher failure rate, with approximately half of all tested configurations failing. Inspection shows that the failures are concentrated at longer spans ($L \ge 4$\,m for ComFlor~60, $L \ge 5$\,m for ComFlor~80) where the model computes a permissible span that slightly exceeds the manufacturer's stated limit (Span/Limit $>$ 1). The principal sources of this overprediction are: (i) the fallback assumption $A_{p,e} = 0.9\,A_p$ for shallow profiles, which does not reflect the test-derived effective area used in the manufacturer software; and (ii) the use of Eurocode~4 design equations rather than the UK National Annex or empirically-adjusted design tables supplied with the ComFlor software, which incorporate additional conservative calibration factors.

\todo[inline]{Add Figure: scatter plot of computed maximum span vs manufacturer-tabulated maximum span. 1:1 line, $\pm$10\% envelope, points coloured by deck family. Annotate the ComFlor 60/80 cluster above the 1:1 line.}

\todo[inline]{check the comflor and kingspan tables in the code, rerun and update results and rewrite this section}
The Multideck N/A cases correspond to propped configurations for which the Kingspan Catalyst software uses BS~5950 design tables that are not reproduced in the technical manual; no independent reference value is available for these cases. The ComFlor~46 deck is similarly N/A for all configurations due to the absence of published design table data at the span range evaluated.

Overall, the validation gives confidence that the structural checks are correctly implemented for deep deck profiles and for the shallow ComFlor~225/51 family. The conservative bias for ComFlor~60/80 at longer spans means these profiles are treated conservatively within the optimisation, which is an acceptable outcome for the purposes of a comparative GWP study.
...
\subsubsection*{Connection assumptions}

\paragraph{Shear connectors not considered.}
End anchorage by welded headed shear studs (per 4-1-1/9.7.4) is not credited in the longitudinal shear resistance, consistent with manufacturer design tables. Comparative analysis (script \texttt{end\_anchorage\_comparison.py}) indicates that the resulting moment capacity increase ranges from approximately \textbf{[INSERT X]}\% to \textbf{[INSERT Y]}\% across the deck range. Including shear studs would also introduce an additional GWP penalty that is not quantified here.
\todo[inline]{Delete this as they are included in code just never needed so not selected - add to design procedure above too.}
...
Tata Steel publish individual EPDs for each ComFlor profile and gauge combination. Kingspan publish a single EPD per tonne of steel for the entire Multideck range. ArcelorMittal Cofraplus and Cofrastra products are included in the database but are excluded from the main optimisation runs: ArcelorMittal publish a single EPD covering all deck profiles regardless of depth or gauge, which does not reflect the substantially different steel quantities per unit area between shallow and deep profiles. After comparing individual manufacturer EPDs with the KBOB reference database, it was determined that the ArcelorMittal single-EPD approach would underrepresent the GWP of deep decking and overrepresent that of shallow decking, introducing a systematic bias that would distort the comparative results. Tata Steel and Kingspan products are therefore retained as the primary optimisation dataset.
\todo[inline]{Expand this paragraph with specific numbers comparing ArcelorMittal EPD vs KBOB values and vs Tata Steel/Kingspan profile-specific data.}
