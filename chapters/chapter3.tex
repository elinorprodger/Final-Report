...
\hline
Construction stage UDL          & $Q_{k,1b}$        & 0.75 kN/m²         & 1-1-6/4.11.2 \\
Construction stage patch load   & $Q_{k,1b,\mathrm{patch}}$ & 1.5 kN/m² over 3$\times$3 m & 1-1-6/4.11.2 \\
\hline
...
Longitudinal shear              & $\gamma_{VS}$     & 1.25               & 4-1-1/9.7.3 \\
\hline
\multicolumn{4}{l}{\textit{Serviceability Limits}} \\
\hline
Construction stage deflection   & $\delta_\mathrm{constr}$  & $\min(L/180, 20\,\mathrm{mm})$ or $\min(L/130, 30\,\mathrm{mm})$    & 4-1-1/9.3.2; UK NA \\
Composite stage --- total       & $\delta_\mathrm{tot}$     & $L/300$                            & SIA 263$^{\ddagger}$ \\
Composite stage --- variable    & $\delta_\mathrm{var}$     & $\min(L/350, 20\,\mathrm{mm})$    & 4-1-1/9.8.2(4) \\
\hline
\multicolumn{4}{l}{\textit{Cross-section Constraints}} \\
\hline
Minimum concrete cover above deck & ---             & 50\,mm                            & 4-1-1/9.2.1 \\
Minimum total depth              & ---             & $h_p + 50$\,mm; not less than 90\,mm & 4-1-1/9.2.1 \\
Concrete cover to top mesh       & $c_\mathrm{nom}$ & 20\,mm                            & 2-1-1/4.4 \\
\hline
...
Creep coefficient                & $\varphi_t$    & EN 1992-1-1 Annex B & 2-1-1/5.4.2.2 \\
Modular ratio (short-term)       & $n_0$          & $E_a / E_{cm}$ & 4-1-1/5.4.2.2 \\
Modular ratio (long-term)        & $n_L$          & $n_0(1 + 1.1\varphi_t)$ & 4-1-1/5.4.2.2 \\
...
\vspace{4pt}\\
{\footnotesize
$^{\ddagger}$ 4-1-1/9.8.2(4) recommends a total deflection limit of $L/250$; the more conservative limit of $L/300$ is adopted here in accordance with SIA~263 \citep{RefWorks:sia263}, for consistency with the prior framework against which the composite results are compared.\\
...
\midrule
Construction & ULS bending (sagging) & 3-1-3/5.4.3, 4-1-1/9.3 \\
Construction & ULS web shear buckling      & 3-1-3/6.1.5 \\
Construction & SLS deflection              & 4-1-1/9.3.2; UK NA \\
Composite    & ULS sagging moment          & 4-1-1/9.7.2 \\
Composite    & ULS longitudinal shear      & 4-1-1/9.7.3 (m--k) or 9.7.5 ($\tau_u$) \\
Composite    & ULS vertical shear          & 4-1-1/9.7.5; 2-1-1/6.2.2 (2004)\\
Composite    & ULS hogging (cont.\ spans)  & 4-1-1/9.7.3(2)P, 9.4.2(3) \\
Composite    & SLS deflection (total \& variable) & SIA 263; 4-1-1/9.8.2(4) \\
...
\paragraph{ULS shear (web buckling).} Web shear buckling resistance per web is computed in accordance with 3-1-3/6.1.5:
\begin{equation}
...
\paragraph{SLS deflection.} The construction-stage SLS deflection check uses the initial elastic deflection of the bare deck under self-weight $G_{k,1a}$ and wet concrete weight $Q_{k,1c}$ only; construction live loads are excluded per 4-1-1/9.3.2(2):
\begin{equation}
...
End anchorage by through-deck welded headed studs (4-1-1/9.7.4) can enhance the longitudinal shear resistance of a composite slab, but is deliberately omitted from the design procedure in this study. The rationale is given in Section~\ref{sec:assumptions}; in brief, end anchorage cannot be applied consistently across the deck database, because its benefit can only be credited for decks characterised by the partial connection ($\tau_{u,Rk}$) method and not for those characterised solely by the m--k constants. The longitudinal shear resistance is therefore taken throughout as the embossment-and-friction shear bond alone, $N_c = \tau_{u,Rd}\,b\,L_x$ (or the m--k resistance of Equation~\eqref{eq:mk}), with no contribution from mechanical connectors. A quantitative assessment of the resistance that end anchorage would have provided, had it been included, is presented in Appendix~\ref{app:end_anchorage}.
...
The two phenomena of concern are airborne sound transmission and impact sound transmission. In Switzerland, SIA~181 \citep{RefWorks:sia181} expresses its requirements using a standardised level difference $D_i$ for airborne sound and a standardised impact sound pressure level $L'_i$ for impact sound, set according to a classification of the sensitivity of the receiving space and the noise exposure.  In the United Kingdom, Approved Document~E \citep{RefWorks:adePartE} specifies field-measured requirements for separating floors for the standardised weighted sound level difference including a spectrum adaptation term ($D_{nT,w}+C_\mathrm{tr}$) and the standardised weighted impact sound pressure level ($L'_{nT,w}$). The design requirements for each occupancy scenario are summarised in Table~\ref{tab:acoustic_requirements}.

\begin{table}[h!]
\small
\centering
\caption{Acoustic design requirements by occupancy scenario}
\label{tab:acoustic_requirements}
\begin{tabular}{lcccc}
\toprule
 & \multicolumn{2}{c}{\textbf{Switzerland (SIA~181)}} & \multicolumn{2}{c}{\textbf{UK}} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Scenario & Airborne $D_i$ & Impact $L'_i$ & Airborne & Impact \\
         & [dB] & [dB] & [dB] & [dB] \\
\midrule
Residential & $\ge 52$--$56$ & $\le 53$--$49$ & $D_{nT,w}+C_\mathrm{tr} \ge 45$ & $L'_{nT,w} \le 62$ \\
Office      & $\ge 52$--$56$ & $\le 53$--$49$ & $D_{nT,w} \ge 45$               & --- \\
Retail      & $\ge 47$       & $\le 58$        & $D_{nT,w}+C_\mathrm{tr} \ge 45$ & $L'_{nT,w} \le 62$ \\
Industrial  & ---            & ---             & ---                             & --- \\
\bottomrule
\end{tabular}
\end{table}

The acoustic performance of each floor build-up was estimated using the SCI publications P321 \citep{RefWorks:RefID:acousticslimdek} and P322 \citep{RefWorks:acousticcomposite}, which provide predicted in-situ performance ranges for a range of composite floor build-up types (screed, platform, raft, and cradle floors). SIA~181 was adopted as the governing standard, since it is the more conservative requirement and sets the binding constraint for the floor build-up choice in all occupancy scenarios where acoustic performance is relevant. The UK Approved Document~E and British Council of Offices (BCO) requirements were used as a cross-check; in all cases the build-up selected for SIA~181 compliance was found to satisfy the UK requirements as well. For each occupancy scenario, the minimum-GWP build-up type whose predicted performance envelope (from SCI~P321/P322) met or exceeded the applicable SIA~181 requirement was identified. Composite slabs with profiled steel decking have lower mass per unit area than solid reinforced concrete slabs of equivalent structural depth, owing to the void volume within the deck ribs. This lower mass reduces the inherent airborne sound insulation of the bare slab relative to solid concrete, and means that supplementary build-up layers are required to satisfy both airborne and impact sound requirements in the residential and office scenarios.

For the \textbf{residential} scenario, a cement screed (85\,mm) over glass wool insulation (30\,mm) with a parquet walking surface (11\,mm) is adopted. From SCI~P322, a screed floor on a composite deck achieves $D_{nT,w}+C_\mathrm{tr} = 50$--$57$\,dB and $L'_{nT,w} = 40$--$50$\,dB, which satisfies the SIA~181 minimum requirement for medium sensitivity ($D_i \ge 52$\,dB, $L'_i \le 53$\,dB) and the UK Approved Document~E requirement ($D_{nT,w}+C_\mathrm{tr} \ge 45$\,dB, $L'_{nT,w} \le 62$\,dB).

For the \textbf{office} scenario, a raised access floor system comprising chipboard panel (18\,mm) over gypsum fibreboard (19\,mm) and rock wool insulation (30\,mm) is adopted. Raised access floors are widely used in commercial fit-out to distribute mechanical and electrical services and are compatible with composite slab construction without structural modification. From SCI~P322, a platform floor construction achieves $D_{nT,w}+C_\mathrm{tr} = 52$--$57$\,dB, meeting the SIA~181 medium-sensitivity airborne requirement ($D_i \ge 52$\,dB) and the UK airborne target ($D_{nT,w} \ge 45$\,dB). No impact requirement applies in the UK office context; the SIA~181 impact requirement ($L'_i \le 53$\,dB) is met by the resilient rock wool layer.

For the \textbf{retail} scenario, a parquet walking surface (11\,mm) over gypsum fibreboard (19\,mm) and glass wool insulation (25\,mm) is adopted. SIA~181 applies a lower (less stringent) sensitivity classification to retail spaces ($D_i \ge 47$\,dB, $L'_i \le 58$\,dB), which is satisfied by this lightweight build-up at a lower GWP cost than the residential screed construction.

For the \textbf{industrial} scenario, a cement screed (85\,mm) over rock wool insulation (30\,mm) is adopted. No inter-floor acoustic performance requirement applies to industrial buildings under SIA~181 or under Approved Document~E; the build-up is selected instead to provide a hardwearing walking surface and minimal additional GWP, with the screed also contributing to the slab's fire performance.
...
\begin{figure}[p]
  \centering
  \resizebox{\textwidth}{!}{\input{code_flowchart}}
...
The outer optimisation loop iterates over all combinations of deck product and concrete grade in the database. Both propped and unpropped optimisations are performed independently for every deck--concrete grade combination; each produces either a feasible optimal depth or records infeasibility across the full search range. The reported optimum is then selected in two stages. First, the minimum-GWP section is identified among all \emph{unpropped} feasible solutions across all deck--concrete combinations. If at least one unpropped feasible solution exists, that section is the reported optimum, regardless of whether a propped configuration with lower GWP might also be available. Only when no unpropped solution is feasible for any deck or concrete grade at a given span does the selection fall back to the minimum-GWP propped section.

This preference for unpropped construction reflects standard practice: unpropped casting avoids the cost, programme impact, and load-path complexity of temporary propping, and is the default assumption for composite slab construction wherever the deck can carry the wet-concrete load unaided. The model therefore never selects a propped solution purely on GWP grounds; propped construction appears in the results only where the construction-stage checks are infeasible for all unpropped deck--concrete combinations at the span in question. The propped/unpropped status of every reported optimum is recorded in the output data and indicated on the GWP--span plots.

It follows that, at spans of 3--5\,m where unpropped construction is feasible, the reported composite GWP figures represent the minimum-GWP \emph{unpropped} solution, not the global GWP minimum across all construction methods. The additional GWP cost of this preference---relative to what propped construction would achieve---is quantified in Section~\ref{sec:res_office_gov}.
...
Local resistance checks at supports per 3-1-3/6.1.7 (web crippling and combined bending plus support reaction) are not currently implemented. These checks are not expected to control for the span and load ranges considered.

\paragraph{End anchorage omitted.}
End anchorage by through-deck welded headed studs (4-1-1/9.7.4) is excluded from the longitudinal shear resistance throughout this study. The omission is a deliberate consistency measure rather than a conservative simplification of convenience. The benefit of end anchorage can only be credited within the partial connection method, which requires the characteristic shear bond strength $\tau_{u,Rk}$; it cannot be credited for decks characterised solely by the m--k constants, because the m--k method is empirical and not based on a mechanical model and therefore cannot separate the contribution of mechanical connectors from the measured shear bond \citep{RefWorks:bode1993modern}. Since the deck database contains products characterised by both methods, crediting end anchorage only for the $\tau_u$ decks would introduce an inconsistent basis of comparison---the same physical enhancement would be rewarded for some products and ignored for others purely because of how each manufacturer chose to report its test data. Omitting end anchorage for all decks restores a consistent basis and yields a conservative longitudinal shear resistance in every case. The resistance forgone by this choice is quantified in Appendix~\ref{app:end_anchorage}, which compares the moment capacity with and without end anchorage across all $\tau_u$ decks and presents a worked example for a representative section; the comparison confirms that end anchorage would increase the partial-connection moment resistance, but that omitting it does not alter the principal comparative conclusions, since longitudinal shear rarely governs the optimal sections (Section~\ref{sec:res_office_gov}).

\subsubsection*{Material and EPD assumptions}

\paragraph{Concrete grade range C25/30 to C30/37.}
Only normal-weight concrete grades C25/30 and C30/37 are considered, as increased compressive strength provides limited benefit for the composite slab configurations at the spans considered and the EPD intensity per kg increases with strength grade. C20/25 is excluded in order to align with industry practice outlined by the technical manuals which all assume either C25/30 or C30/37 - see Section~\ref{sec:disc_c20} for further discussion.

\paragraph{Granularity of deck EPDs.}
Tata Steel publishes individual EPDs per unit area (kg\,CO$_2$eq/m$^2$) for each ComFlor profile and gauge combination, giving profile-specific values that correctly reflect the variation in steel mass per m$^2$ across the range. Converting to a per-kilogram basis yields a GWP of 2.79--3.09\,kg\,CO$_2$eq/kg across the ComFlor product range included in the database. Kingspan publishes a single EPD declared per tonne of steel, applicable to the entire Multideck range, giving a constant 2.50\,kg\,CO$_2$eq/kg. Although this is a product-range aggregate rather than a profile-specific declaration, the per-tonne basis is physically consistent: heavier profiles automatically attract proportionally higher GWP, so the single EPD does not introduce a systematic bias between light and heavy gauges.

ArcelorMittal Cofraplus and Cofrastra products are retained in the database but excluded from the main optimisation runs. ArcelorMittal publishes a single EPD declared per unit area (18.0\,kg\,CO$_2$eq/m$^2$) applicable to all deck profiles regardless of depth or gauge. This relationship is physically implausible: a heavier profile requires more steel and more manufacturing energy per m$^2$, so its GWP per m$^2$ should be higher, not equal. The consequence is that deep, heavy decking profiles appear substantially greener under the ArcelorMittal EPD than under independent references. The KBOB reference value for galvanised sheet steel is 3.64\,kg\,CO$_2$eq/kg \citep{RefWorks:kbob2022}, further contextualising the ArcelorMittal implied values as severely underestimated across all but the very lightest gauges. Including ArcelorMittal products in the optimisation would therefore produce a systematic bias in favour of deep composite slabs --- precisely the configuration where the EPD underestimation is most severe --- distorting the comparative GWP rankings in a way that cannot be corrected without product-specific declarations. Tata Steel and Kingspan products are accordingly retained as the primary dataset.
